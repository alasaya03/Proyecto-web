...
<%
    ...
    if (nombre == null || email == null || contra == null || telf == null) {
        response.sendRedirect("registrotutor.html?error=invalid_params");
        return;
    }
    ...
    try {
        ...
        if (rs.next()) {
            response.sendRedirect("registrotutor.html?error=email_exists");
            rs.close();
            check.close();
            con.close();
            return;
        ...
        // Insertar
        String sql = "INSERT INTO tutor (nom_tutor, telefono, correo, contraseña) VALUES (?, ?, ?, ?)";
        PreparedStatement st = con.prepareStatement(sql);

        st.setString(1, nombre);
        st.setString(2, telf);
        st.setString(3, email);
        st.setString(4, passHash);
        ...
        if (filas > 0) {
            ...
            response.sendRedirect("registrotutor.html?error=unknown");
        }

    } catch (Exception e) {
        response.sendRedirect("registrotutor.html?error=server");
    }
%>
