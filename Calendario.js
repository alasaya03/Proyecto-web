...
function initCalendar() {
    ...
    // Intentar obtener ID_VETE (puede ser hidden o select)
    const veteElement = document.getElementById("ID_VETE");
    if (veteElement) {
        ID_VETE = parseInt(veteElement.value);

        // 🔥 IMPORTANTE: actualizar cuando cambie (caso tutor)
        veteElement.addEventListener("change", function () {
            ID_VETE = parseInt(this.value);

            // Recargar horas si ya hay fecha seleccionada
            if (selectedDate) {
                loadHours();
            }
        });
...
// ============================
function loadHours() {
    if (!ID_VETE || isNaN(ID_VETE)) {
        timeButtonsContainer.innerHTML = "<p>Selecciona un veterinario primero</p>";
        return;
    }
    ...
    fetch(`getHoras.jsp?fecha=${selectedDate}&id_vete=${ID_VETE}`)
        ...
        .catch(err => {
            ...
            timeButtonsContainer.innerHTML = "<p>Error cargando horarios</p>";
        });
}
